...
def make_xml():
    ...
    ET.SubElement(default, "geom", friction="1 .005 .0001", condim="3",
    ...
    ET.SubElement(world, "geom", name="ground", type="plane", size="100 100 .1",
    ...
    while pending:
        for name in pending[:]:
            ...
            ET.SubElement(body, "geom", name=name, type="mesh", mesh=name,
                          mass=str(cfg["mass"]), contype="0" if name.endswith("wing") else "2",
            ...
            if leg in LEGS:
                ...
                for axis in axes:
                    ...
                    ET.SubElement(body, "joint", name=joint_name(name, axis), type="hinge",
                                  ...
                                  range=vector([rest - span, rest + span]),
        ...
        for link, axis in ACTIVE_DOF:
            ...
            for sign, suffix in [(-1, "positive"), (1, "negative")]:
                ET.SubElement(actuators, "muscle", name=f"{name}_{suffix}", joint=name,
                              ...
                              force=str(MAX_FORCE), timeconst=".002 .004", fpmax=".00001")
...
class FlyBody:
    ...
    def reset(self):
        ...
        self.data.qpos[2] += .03 - np.min(self.data.site_xpos[self.foot_ids, 2])
    ...
    def _separate_reset_contacts(self):
        ...
        for _ in range(80):
            ...
            for contact in d.contact:
                if contact.geom1 == 0 or contact.geom2 == 0 or contact.dist >= .001:
                    continue
    ...
    def step(self, motor, dt=.02):
        ...
        for step in range(round(dt / h)):
            ...
            if step % 10 == 0:
                ...
                activation = np.stack((np.maximum(torque, 0), np.maximum(-torque, 0)), axis=1) / (MAX_FORCE * MOMENT_ARM)
                activation = np.clip(activation + .025, 0, 1)
                activation *= np.repeat(enabled, 7)[:, None]
                self.data.ctrl[:] = activation.ravel()
    ...
    def step_muscles(self, excitation, dt=.02):
        """Direct RL control: bypass the reference tracker and command all 84 muscles."""
        excitation = np.asarray(excitation, dtype=float)
        if excitation.shape != (84,) or not np.isfinite(excitation).all():
            raise ValueError("Expected 84 finite muscle excitations")
    ...
    def snapshot(self):
        ...
        grouped_activation = self.data.act.reshape(6, 7, 2).mean(axis=1)
        grouped_force = np.abs(self.data.actuator_force).reshape(6, 7, 2).sum(axis=1)
        ...
        return {
            "bodies": [{"name": name, "parent": SEGMENTS[name]["parent"], "mesh": mesh_name(name),
                        ...
                        "quaternion": self.data.xquat[self.body_ids[name]].tolist()} for name in self.names],
            ...
            "anatomy": {"name": "NeuroMechFly articulated body", "segments": len(self.names),
                        "joints": self.model.njnt - 1, "actuated_joints": 42, "muscles": 84,
