...
def make_engine(tmp_path, delay):
    ...
    engine = MetalGradientBrain(brain, adapter, NeuromuscularBridge(brain))
    engine.sync_weights()
    return engine


@pytest.mark.parametrize('delay', [0., 1.8])
def test_fp16_forward_matches_existing_gpu_inference(tmp_path, delay):
    engine = make_engine(tmp_path, delay)
    b = engine.brain
...
def test_fp32_backward_matches_independent_dense_autograd(tmp_path, delay, full_history, stable_history):
    engine = make_engine(tmp_path, delay)
    b = engine.brain
    ...
    for t,(observed, flags) in enumerate(zip(raw_history,flags_history)):
        ...
        spikes = Spike.apply(raw+45)*free
...
def test_global_norm_clipping_preserves_extreme_gradient_direction():
    from types import SimpleNamespace
    from flylab.gesture_history import clip_full_gradient, scale_power_two
    parameter = torch.nn.Parameter(torch.zeros(2))
    parameter.grad = torch.tensor([3e30,4e30])
    engine = SimpleNamespace(parameters=parameter, gradient_exponent=500)
    norm, log_norm = clip_full_gradient(engine)
    torch.testing.assert_close(parameter.grad,torch.tensor([.6,.8]))
    assert norm > 1e180 and log_norm > 180 and engine.gradient_exponent == 0
    torch.testing.assert_close(scale_power_two(torch.tensor([1e-30]),150),torch.tensor([1e-30*2.**150]),rtol=1e-5,atol=0)
